...
Use the helper to auto-minimize conflicts first:
...
The script rebases with `-X theirs` to auto-resolve as many conflicts as possible.

If any conflicts remain, use:

```bash
git status --short
git checkout --theirs index.html app.js styles.css README.md
git add index.html app.js styles.css README.md
# resolve any remaining non-UI files
...
./scripts/resolve-pr-conflicts.sh origin main
```
## Opening a fresh PR after closing old ones
- Open a new PR from branch `work` to `main` after pushing latest commits.
- If GitHub still reports conflicts, run the conflict helper flow in this README, then push with `--force-with-lease` and refresh the PR page.
